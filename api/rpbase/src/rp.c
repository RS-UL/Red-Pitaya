...
int rp_AIpinGetValueRaw(int unsigned pin, uint32_t* value) {
	...
	case 0:
		fp =
				fopen(
						"/sys/devices/soc0/amba_pl/83c00000.xadc_wiz/iio:device1/in_voltage11_vaux8_raw",
						"r");
		break;
	case 1:
		fp =
				fopen(
						"/sys/devices/soc0/amba_pl/83c00000.xadc_wiz/iio:device1/in_voltage9_vaux0_raw",
						"r");
		break;
	case 2:
		fp =
				fopen(
						"/sys/devices/soc0/amba_pl/83c00000.xadc_wiz/iio:device1/in_voltage10_vaux1_raw",
						"r");
		break;
	case 3:
		fp =
				fopen(
						"/sys/devices/soc0/amba_pl/83c00000.xadc_wiz/iio:device1/in_voltage12_vaux9_raw",
						"r");
		break;
	default:
		return RP_EPN;
...
int rp_AIpinGetValue(int unsigned pin, float* value) {
	...
	*value = (((float) value_raw / ANALOG_IN_MAX_VAL_INTEGER)
			* (ANALOG_IN_MAX_VAL - ANALOG_IN_MIN_VAL)) + ANALOG_IN_MIN_VAL;
	return result;
}
...
int rp_AOpinGetValue(int unsigned pin, float* value) {
	...
	*value = (((float) value_raw / ANALOG_OUT_MAX_VAL_INTEGER)
			* (ANALOG_OUT_MAX_VAL - ANALOG_OUT_MIN_VAL)) + ANALOG_OUT_MIN_VAL;
	return result;
}
...
int rp_AcqGetDataPosRaw(rp_channel_t channel, uint32_t start_pos,
		uint32_t end_pos, int16_t* buffer, uint32_t* buffer_size) {
...
int rp_CounterSetGatedCounting(bool gatedCounting) {

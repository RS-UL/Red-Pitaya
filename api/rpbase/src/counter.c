...
int counter_SetGatedCounting(bool gatedCounting) {
	return cmn_SetShiftedValue(&counter_reg->config, gatedCounting ? 1 : 0,
			COUNTER_CONFIG_GATED_COUNTING_MASK
					>> COUNTER_CONFIG_GATED_COUNTING_BIT_OFFSET,
			COUNTER_CONFIG_GATED_COUNTING_BIT_OFFSET);
}
int counter_GetGatedCounting(bool *gatedCounting) {
	return cmn_GetShiftedValue(&counter_reg->config, (uint32_t*) gatedCounting,
			COUNTER_CONFIG_GATED_COUNTING_MASK
					>> COUNTER_CONFIG_GATED_COUNTING_BIT_OFFSET,
			COUNTER_CONFIG_GATED_COUNTING_BIT_OFFSET);
}
...
int counter_GetBinDataBuffers(
		const volatile uint32_t *buffers[COUNTER_NUM_COUNTERS]) {
	for (int i = 0; i < COUNTER_NUM_COUNTERS; i++)
		buffers[i] = counter_bin_data[i];
	return RP_OK;
}
...
int counter_ResetBinDataBuffers() {
	return counter_ResetBinDataBuffersPartially(COUNTER_BINS);
}

...
static const int COUNTER_BASE_ADDR = 0x40600000;
...
static const int COUNTER_BINS_CH2_OFFSET = 0x00018000;
...
static const int COUNTER_REG_CONTROL_MASK		= 0x00000007;
...
typedef enum {
	...
	countImmediately, countTriggered, trigger
} counter_control_cmd;

typedef enum {
	idle = 0, immediateCounting_start,
...
int counter_SetGatedCounting(bool countingGated);
int counter_GetGatedCounting(bool *countingGated);
int counter_GetBinAddress(uint32_t *binAddress);
int counter_GetRepetitionCounter(uint32_t *repetitionCounter);
int counter_GetBinDataBuffers(const volatile uint32_t *buffers[COUNTER_NUM_COUNTERS]);
int counter_ResetBinDataBuffersPartially(uint32_t numBins);
int counter_ResetBinDataBuffers();
